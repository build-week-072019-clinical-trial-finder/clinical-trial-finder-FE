...
export const login = (history, credentials) => dispatch => {
  ...
  axios
    .post("https://clincal-trials.herokuapp.com/api/login", credentials)
    .then(response => {
...
export const fetch = searchTerms => dispatch => {
  ...
  axios
    ...
    .then(response => {
      ...
      let ids = Array(response.data[keys[0]].length)
        .fill()
      ...
      keys.forEach(
        key =>
          (data = data.map(item => ({
            ...item,
            [key]: response.data[key][item["trial_id"]]
          })))
      );
      console.log(data);
      dispatch({
...
export const fetchWatchlist = () => dispatch => {
  ...
  axiosWithAuth()
    .get(`https://clincal-trials.herokuapp.com/api/users/${userId}/watchlist`)
    .then(response => {
...
export const addToWatchlist = trial => dispatch => {
  ...
  axiosWithAuth()
    .post(`https://clincal-trials.herokuapp.com/api/watchlist`, trialToAdd)
    .then(response => {
      console.log("fetch watchlist success: ", response);
      dispatch({
      ...
      return axiosWithAuth()
        .get(
          `https://clincal-trials.herokuapp.com/api/users/${userId}/watchlist`
        )
        .then(response => {
          console.log("fetch watchlist success: ", response);
          dispatch({
...
export const removeFromWatchlist = trial => dispatch => {
  ...
  axiosWithAuth()
    .delete(`https://clincal-trials.herokuapp.com/api/watchlist/${trial.id}`)
    .then(response => {
      console.log("fetch watchlist success: ", response);
      dispatch({
